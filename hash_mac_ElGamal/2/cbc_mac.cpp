...
uint8_t* AES_Encrypt(char *plainFile, uint8_t key[],int size){ //size爲aes的密鑰的大小只能爲128位，key位aes的密鑰
	FILE *plain;  
    int count;  
    int m=0;
    int tag=-1;
	...
	if((count = fread(in,sizeof(char),16,plain)) == 16){  
		for(i=0;i<16;i++)
    			in[i]^=key[i];	
...
uint8_t* expansion(uint8_t in1[],uint8_t key[]){   //後三十位用不同密鑰加密之後的32位得到
	static uint8_t out[20];
    	uint8_t inTemp[16];
	uint8_t *w; // expanded key
	int i;
	w = (uint8_t*)malloc(Nb*(Nr+1)*4);
		key_expansion(key, w);
	cipher(in1 /* in */, inTemp /* out */, w /* expanded key */); 
	for(i=0;i<16;i++){
		out[i]=in1[i];	
}
	for(i=16;i<20;i++){
...
*/
		...
		uint8_t key[] = {		 //aes初始秘钥,只能位128位(常規的密鑰)
		0x00, 0x01, 0x02, 0x03, 
		0x04, 0x05, 0x06, 0x07, 
		0x08, 0x09, 0x0a, 0x0b, 
		0x0c, 0x0d, 0x0e, 0x0f};
		uint8_t key1[] = {		 //aes初始秘钥,只能位128位（擴展用的密鑰）
		0x10, 0x11, 0x12, 0x13, 
		0x14, 0x15, 0x16, 0x17, 
		0x18, 0x19, 0x1a, 0x1b, 
		0x1c, 0x1d, 0x1e, 0x1f};
	...
	size=sizeof(key);
	uint8_t* out;
	out=AES_Encrypt("1.txt",key,size);           //沒有擴張得到的結果，是128位的結果
	result=expansion(out,key1);
	printf("out:\n");
	for (i = 0; i < 5; i++) {
		printf("%x %x %x %x ", result[4*i+0], result[4*i+1], result[4*i+2], result[4*i+3]);
	}
